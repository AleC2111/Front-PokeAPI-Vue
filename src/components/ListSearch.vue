<script setup>
//Fase 3
// Colocar más información en los resultados con Promise.all
// Buscar el pokemon al hacer click encima
// Paginación
...
const availableTypes = ["fire", "water", "grass", "electric", "ground", "rock", "poison", "ghost",
    "fighting", "psychic", "steel", "normal", "flying", "dragon", "ice", "dark", "fairy", "bug"]
const maxCount = 1351
let nextPage = null
...
function parseData(data){
    ...
    let pokemonFound = []
    data.results.forEach(found => { pokemonFound.push(found.name) });
...
function parseTypeData(data, offset, limit){
    ...
    sliced.forEach(found => { pokemonFound.push(found.pokemon.name) });
...
function searchParameters(){
    try{
        let offset
        let typeSearching = types_option.value.value

        if (offset_option.value.value === null || offset_option.value.value === ""){
            offset = 0
        } else {
...
<template>
    ...
    <div ref="options" class="text-container">
        <input id="offset" ref="offset_option" type="integer" placeholder="Id de inicio de la busqueda">
        <input id="types" ref="types_option" placeholder="Tipo a buscar">
        <button ref="confirm" @click="searchParameters">Confirmar</button>
    </div>

    <hr>
    <div ref="list_content" class="text-container" style="display: inline-block;">
